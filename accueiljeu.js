...
function checkLocalStorage() {
  ...
  const inputEmail = document.getElementById('angelin');
  const inputNickName = document.getElementById('angelin2');
  const localValue = document.getElementById('localValue');

  if (nickname && email) {
    inputEmail.style.display = 'none';
    inputNickName.style.display = 'none';
    localValue.textContent = `Hi! ${nickname} - ${email}`;
  } else {
  ...
  try {
    ...
    const response = await fetch('http://localhost:1337/api/game-projets', {
      ...
      headers: {
        ...
        Authorization:
          'Bearer ',
      },
      body: JSON.stringify({
        data: {
          email,
...
const fetchAndSortScores = async () => {
  try {
    ...
    if (leaderboard) {
      leaderboard.innerHTML = sortedScore
        .map(
          (player, index) => `
                <tr>
                    <td>${index + 1}</td>
